...
from app.models import Contact, CrawlLog, SearchJob
...
@router.get("/searches/new", response_class=HTMLResponse, dependencies=[Depends(require_admin)])
...
@router.get("/searches/{job_id}", response_class=HTMLResponse, dependencies=[Depends(require_admin)])
def search_detail(job_id: str, request: Request, db: Session = Depends(get_db)):
...
def contacts_page(
    ...
    conditions = []
    if q:
        conditions.append(or_(Contact.organization.ilike(f"%{q}%"), Contact.domain.ilike(f"%{q}%")))
    if country:
    ...
    contacts = list(db.scalars(
        statement.order_by(Contact.verified_at.desc(), Contact.id.desc()).offset((page - 1) * page_size).limit(page_size)
    ))
    countries = [value for value in db.scalars(select(Contact.country).distinct().order_by(Contact.country)) if value]
...
def upload_xlsx(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        temp_path.unlink(missing_ok=True)
    return RedirectResponse(f"/?imported={imported}&duplicates={duplicates}", status_code=303)


@router.get("/exports/{format}", dependencies=[Depends(require_admin)])
def download_export(format: str, job_id: str | None = None, db: Session = Depends(get_db)):
    if format == "csv":
...
@router.get("/api/searches/{job_id}", response_model=SearchRead, dependencies=[Depends(require_admin)])
